...
export const Dashboard = () => {
  ...
  const pivotedData = useMemo(() => {
    ...
    data?.forEach((row) => {
      ...
      checkedSummaryColumns.forEach((column, index) => {
        const colValue = row[column];

        if (!(colValue in currentRoot)) currentRoot[colValue] = {};

        currentRoot = currentRoot[colValue];

        if (index === checkedSummaryColumns.length - 1)
          currentRoot[pivotValue] = {};
      });
    });

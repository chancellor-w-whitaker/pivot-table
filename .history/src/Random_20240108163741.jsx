...
function useData(url, onCanSetData) {
  ...
  useEffect(() => {
    if (url) {
      ...
      fetch(url)
        ...
        .then((json) => {
          if (!ignore) {
            typeof onCanSetData === "function"
              ? onCanSetData(json, setData)
              : setData(json);
          }
        });
...
const useData = (url, onBeforeDataSet) => {
  const [data, setData] = useState(null);
  ...
  useEffect(() => {
    if (url) {
      ...
      fetch(url)
        ...
        .then((json) => {
          if (!ignore) {
            typeof onBeforeDataSet === "function"
              ? onBeforeDataSet(json, setData)
              : setData(json);
          }
        });

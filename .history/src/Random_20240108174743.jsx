...
export const Random = () => {
  ...
  const onJsonReceived = useCallback(
    (rows, setData) => {
      ...
      // text fields you can group by
      const fieldsToGroupBy = typedColumns
        .filter(({ type }) => type === "string")
        .map(({ field }) => field);

      // numeric fields you can sum on
      const fieldsToSummarize = typedColumns
        .filter(({ type }) => type === "number")
        .map(({ field }) => field);

      setData({ fieldsToSummarize, fieldsToGroupBy, rows });
    },
    [pivotField]
  );

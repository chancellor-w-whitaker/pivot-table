...
export const Dashboard = () => {
  ...
  const filteredData = useMemo(() => {
    return !isLengthyArray(data) || !filtersState
      ? []
      : data.filter((row) => {
          for (const key of Object.keys(filtersState)) {
            const value = row[key];

            if (!filtersState[key].has(value)) return false;
          }
  ...
  const {
    ...
    () =>
      !isLengthyArray(filteredData)
        ? {}
        : deriveValuesFromData(filteredData, pivotColumn),
    [filteredData, pivotColumn]
  );
  ...
  const { pivotedData, chartData } = useMemo(
    ...
    [filteredData, pivotColumn, measureOptions, checkedSummaryColumns]
  );

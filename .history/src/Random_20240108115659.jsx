...
export const Random = () => {
  ...
  const onWillSetData = useCallback(
    (json, setData) => {
      ...
      // get measure columns
      const measureColumns = typedColumns.filter(
        ({ field, type }) => type === "number" && field !== pivotField
      );

      setData({ summaryColumns, measureColumns, json });
    },
    [pivotField]
  );
...
const useData = (url, willSetDataHandler) => {
  ...
  useEffect(() => {
    if (url) {
      ...
      fetch(url)
        ...
        .then((json) => {
          if (!ignore) {
            typeof willSetDataHandler === "function"
              ? willSetDataHandler(json, setData)
              : setData(json);
          }
        });
